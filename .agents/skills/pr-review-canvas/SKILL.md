...
Read `styles.css`, `renderer.js`, and `template.html` from this skill directory (`.agents/skills/pr-review-canvas/`). These give you a prebuilt dark-themed toolkit. Inject them into the final HTML verbatim.
...
skill_dir = Path('.agents/skills/pr-review-canvas')
...
3. Run the Python assembly script from step 3 above, reading `styles.css`, `renderer.js`, and `template.html` from this skill directory. Write the final HTML to a durable path such as `/tmp/pr-review-{number}.html` or `./pr-review-{number}.html`.
4. Deliver the result to the user:
   - If the agent environment supports opening local HTML, serve it on a fixed port:
     ```bash
     cd /tmp && python3 -m http.server 8432 --bind 127.0.0.1
     ```
     Then open `http://127.0.0.1:8432/pr-review-{number}.html`.
   - Otherwise, tell the user the path to the generated HTML file and let them open it.
